...
SKILLS_SOURCE_DIR = PACKAGE_DIR / "sandbox_skills"
DOCKER_SANDBOX_DIR = PACKAGE_DIR / "docker_sandbox"
...
async def run_workflow(
    ...
    *,
    ...
    async for event in stream_workflow(workflow_id, text_inputs, file_inputs, debug=debug, run_id=run_id):
...
def _sandbox_image_reference() -> tuple[str, bool]:
    configured_image = os.getenv("SANDFLOW_SANDBOX_IMAGE", "").strip()
    if configured_image:
